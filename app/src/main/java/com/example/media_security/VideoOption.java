...
public class VideoOption extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (auth.getCurrentUser() != null) {
            fstore.collection("users").document(auth.getCurrentUser().getUid()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    ...
                    if (task.getResult().exists()) {
                        ...
                        firebaseDatabase = FirebaseDatabase.getInstance().getReference(folder).child("Videos");
            ...
            Toast.makeText(VideoOption.this, "Login OR create account", Toast.LENGTH_SHORT).show();
        }
    ...
    public void adding() {
        ...
        if(ContextCompat.checkSelfPermission(VideoOption.this, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            if (auth.getCurrentUser() != null) {
                fstore.collection("users").document(auth.getCurrentUser().getUid()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                ...
                Toast.makeText(VideoOption.this, "Email authentication require", Toast.LENGTH_LONG).show();
            }
        }
        else{
            ActivityCompat.requestPermissions(VideoOption.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 100);
        }
    }
...
//            }
    ...
    public void uploadVideo() {
        ...
        dialog.show();
        Toast.makeText(VideoOption.this, " Wait until video is uploading....", Toast.LENGTH_SHORT).show();
        ...
        if(auth.getCurrentUser()!=null) {
            fstore.collection("users").document(auth.getCurrentUser().getUid()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    ...
                    if (task.getResult().exists()) {
                        ...
                        upload_video = reference.putFile(Uri.parse(uri));

                        Task<Uri> urlTask = upload_video.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                            @Override
                            ...
                            public void onComplete(@NonNull Task<Uri> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    Video uploadVideo = new Video(encryptedText, downloadUri.toString());
                                    firebaseDatabase.child(firebaseDatabase.push().getKey()).setValue(uploadVideo);
                                    Toast.makeText(VideoOption.this, "Video  uploaded \n Refresh the videos", Toast.LENGTH_SHORT).show();
                                    dialog.dismiss();

                                }
                                else{
                                    Toast.makeText(VideoOption.this, "network error\nTry again", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(VideoOption.this, " Email Authentication is required ", Toast.LENGTH_SHORT).show();
            dialog.dismiss();
        }
    }
                ...
                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                    ...
                    if (data != null) {
                            if (!(number1.getText().toString().contains(data.get("encryptedUri").toString().substring(86, 109)))) {
                                number1.append("video: " + data.get("encryptedUri").toString().substring(86, 109) + "\n\n");
                        }
                    }else{
                        Toast.makeText(getApplicationContext() , "upload videos" , Toast.LENGTH_LONG).show();
                    }
                }
    ...
    public  void decryption(){
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(VideoOption.this);
        ...
        Toast.makeText(VideoOption.this, "Enter the private key to decrypt the data ", Toast.LENGTH_LONG).show();
        ...
        ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(auth.getCurrentUser()!=null) {
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            if (task.getResult().exists()) {
                                if (editText.getText().toString().equals(key)) {
                                    Intent intent = new Intent(VideoOption.this, VideoGallery.class);
                                    startActivity(intent);
                                } else {
                                    Toast.makeText(VideoOption.this, " Incorrect key !! ", Toast.LENGTH_LONG).show();
                                }
                            }
                    ...
                    Toast.makeText(VideoOption.this, "Login OR create account ", Toast.LENGTH_LONG).show();
                }
            }
        });
